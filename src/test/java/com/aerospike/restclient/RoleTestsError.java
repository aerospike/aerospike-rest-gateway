...
public class RoleTestsError {
	...
	public void getRoleThatDoesNotExist() throws Exception {
		mockMVC.perform(get(endpoint + "/" + "NotARealRole"))
		.andExpect(status().isNotFound());
	}

	@Test
	public void deleteRoleThatDoesNotExist() throws Exception {
		mockMVC.perform(delete(endpoint + "/" + "NotARealRole"))
		.andExpect(status().isNotFound());
	}
	...
	public void createRoleThatAlreadyExists() throws Exception {
		...
		String createRoleContent = objectMapper.writeValueAsString(
				new RestClientRole(createdRole));

		mockMVC.perform(post(endpoint)
				.contentType(MediaType.APPLICATION_JSON)
				.content(createRoleContent))
		.andExpect(status().isConflict());
	}

	@Test
	public void
	grantPrivilegeToNonExistentRole() throws Exception {
		...
		mockMVC.perform(post(endpoint + "/" + "NotARealRole" + "/privilege")
				.contentType(MediaType.APPLICATION_JSON)
	...
	@Test
	public void
	revokePrivilegeFromNonExistentRole() throws Exception {
		RestClientPrivilege[] privAry = new RestClientPrivilege[1];
		privAry[0] = new RestClientPrivilege(readWriteTestProd);

		String privsContent = objectMapper.writeValueAsString(privAry);
		mockMVC.perform(patch(endpoint + "/" + "NotARealRole" + "/privilege/delete")
				.contentType(MediaType.APPLICATION_JSON)
				.content(privsContent))
		.andExpect(status().isNotFound());
	}
