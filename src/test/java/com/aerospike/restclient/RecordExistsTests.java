...
public class RecordExistsTests {
	...
	void addParams(boolean useSet) {
		if (useSet) {
			...
			nonExistentEndpoint = ASTestUtils.buildEndpoint("kvs", "test",  "junit", "notreal");
			...
			nonExistentEndpoint = ASTestUtils.buildEndpoint("kvs", "test", "notreal");
	...
	public void testWithNonExistentRecord() throws Exception {
		mockMVC.perform(
				head(nonExistentEndpoint)).andExpect(status().isNotFound());
	}
	...
	public void TestIntegerKey() throws Exception {
		...
		mockMVC.perform(
				head(intEndpoint)).andExpect(status().isOk());
	}
	...
	public void ExistsWithDigest() throws Exception {
		...
		mockMVC.perform(
				head(digestEndpoint)).andExpect(status().isOk());
	}
