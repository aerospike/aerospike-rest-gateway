...
public class SindexTestsCorrect {
	...
	public void createIndex() throws Exception {
		...
		List<RestClientIndex>existingIndexes = idxInfoMaps.stream().map(si -> new RestClientIndex(si)).collect(Collectors.toList());
	...
	private boolean indexMatches(RestClientIndex rcIndex1, RestClientIndex rcIndex2) {
		...
		if (rcIndex1.getSet() == null) {
			if (rcIndex2.getSet() != null) {
				return false;
			}
		} else if (!rcIndex1.getSet().equals(rcIndex2.getSet())) {
			return false;
		}
...
interface RestIndexHandler {
	...
	List<RestClientIndex> getIndexes(MockMvc mockMVC, String endpoint) throws UnsupportedEncodingException, Exception;
}
...
class MsgPackIndexHandler implements RestIndexHandler {
	...
	public Map<String, String> getIndexStats(MockMvc mockMVC, String endpoint) throws Exception {
		...
		Map<String, String>restIndexStatMap = mapper.readValue(resBytes, SindexTestsCorrect.mapStringStringType);

		return restIndexStatMap;
	}
	...
	@Override
	public List<RestClientIndex> getIndexes(MockMvc mockMVC, String endpoint) throws UnsupportedEncodingException, Exception {
		byte[] resBytes = mockMVC.perform(get(endpoint).accept(mediaType))
				.andExpect(status().isOk()).andReturn().getResponse().getContentAsByteArray();

		List<RestClientIndex>indexList = mapper.readValue(resBytes, SindexTestsCorrect.listIndexType);
		return indexList;
	}
}
...
class JSONIndexHandler implements RestIndexHandler {
	...
	public Map<String, String> getIndexStats(MockMvc mockMVC, String endpoint) throws Exception {
		...
		Map<String, String>restIndexStatMap = mapper.readValue(resJson, SindexTestsCorrect.mapStringStringType);
	...
	public List<RestClientIndex> getIndexes(MockMvc mockMVC, String endpoint) throws Exception {
		...
		List<RestClientIndex>indexList = mapper.readValue(resJson, SindexTestsCorrect.listIndexType);
		return indexList;
	}
