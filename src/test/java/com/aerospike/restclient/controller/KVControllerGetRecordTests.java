...
public class KVControllerGetRecordTests {
	...
	private String[] binAry = {"bin1", "bin2", "bin3"};
	private List<String> binList = Arrays.asList(binAry);
	...
	public void testNSSetKeyReturn() {
		...
		RestClientRecord actualRecord = controller.getRecordNamespaceSetKey(ns, set, key, queryParams);
	...
	public void testBinsNSKeyReturn() {
		when(recordService.fetchRecord(
				...
				any(),
				any(Policy.class))).thenReturn(testRecord);
		RestClientRecord actualRecord = controller.getRecordNamespaceKey(ns, key, queryParams);
		Assert.assertTrue(recordsEqual(testRecord, actualRecord));
	}
